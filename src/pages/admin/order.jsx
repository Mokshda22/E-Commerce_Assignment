  ...
  return (
    <div className="flex">
      ...
      <div className="flex-1 p-8 ml-[5rem] lg:ml-64 bg-amber-100 min-h-screen">
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-red-800 mb-4">Order Management</h1>
          <div className="w-full max-w-md mx-auto">
            <div className="relative">
              <input
                type="text"
                placeholder="Search by order ID or customer name..."
                className="w-full pl-10 pr-4 py-3 border-2 bg-red-500 border-pink-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-white"
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
              />
              <Search className="absolute left-3 top-3.5 text-white" size={20} />
            </div>
          </div>
        </div>
          ...
          <table className="min-w-full table-auto">
              ...
              <tr>
                {['orderId', 'date', 'time', 'name', 'email', 'price', 'status'].map((key) => (
                  <th 
                    ...
                    className="px-6 py-3 text-left text-xs font-medium text-white uppercase tracking-wider cursor-pointer hover:bg-pink-200 transition"
                  >
            ...
            <tbody className="bg-red-500 divide-y divide-gray-200">
              {filteredOrders.map((order) => (
                <tr key={order.orderId} className="hover:bg-red-600 transition">
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-white0">
                    {order.orderId}
                  </td>
